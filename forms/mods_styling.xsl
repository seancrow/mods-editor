...
-->
	<xsl:variable name="dots" select="string-length(@id) - string-length(translate(@id, '.', ''))"/>
	  ...
	  <xsl:variable name="repeater">
		  <xsl:choose>
			<xsl:when test="contains(@id, 'insertWidget')"><xsl:value-of select="substring-before(@id, '.')"/></xsl:when>
			<xsl:when test="contains(@id, 'deleteWidget')"><xsl:value-of select="substring-before(@id, '.')"/></xsl:when>
			<xsl:when test="starts-with(@id, 'add-to-')"><xsl:value-of select="substring-after(@id, 'add-to-')"/></xsl:when>
			<xsl:otherwise>error</xsl:otherwise>
		</xsl:choose>
		</xsl:variable>
  ...
  <!-- thumbnail images -->
  <xsl:template match="fi:field[fi:styling/@type='imglink']" >
    <xsl:if test="fi:value != ''">
      <div class="thumbdiv">
        <a href="{fi:value}" target="_new">
          <xsl:apply-templates select="../fi:field[fi:styling/@type='img']" mode="link"/>
        </a>
      </div>
    </xsl:if>
  </xsl:template>
  <xsl:template match="fi:field[fi:styling/@type='img']" mode="link">
    <img class="thumb" src="{fi:value}" alt="thumbnail"/>
  </xsl:template>
  <xsl:template match="fi:field[fi:styling/@type='img']"/>
